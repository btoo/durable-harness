...
export function validateMcpUrl(url: string, allowLocalHttp = false): URL {
  ...
  try {
    ...
    throw new HarnessFault("INVALID_INPUT", "Enter a complete HTTPS MCP server URL.");
  }
  invariant(
  ...
  invariant(
    ...
    "INVALID_INPUT",
    "Use a public HTTPS MCP endpoint. Local HTTP is restricted to the test adapter.",
  );
  return parsed;
}
...
export class Connections {
  ...
  async add(
    principal: Principal,
    input: { spaceId: string; name: string; url: string; auth: McpConnection["auth"] },
    credentials?: McpCredentials,
    ...
    const connection: McpConnection = {
      ...
      state: "connecting",
      backgroundAccess:
        credentials?.refreshToken || input.auth === "bearer" || input.auth === "none"
          ? "supported"
          : "unknown",
      allowedTools: [],
      tools: [],
  ...
  read(principal: Principal, id: string): McpConnection {
    ...
    invariant(
      connection && this.policy.permits(principal, connection.spaceId, "read"),
      "NOT_FOUND",
      "No accessible connection exists with that reference.",
    );
    return connection;
  }
  list(principal: Principal, spaceId: string): McpConnection[] {
    ...
    return this.store
      ...
      .filter((connection) => connection.spaceId === spaceId);
  }
  async discover(principal: Principal, id: string): Promise<McpConnection> {
    ...
    invariant(
      ...
      "RECONNECTION_REQUIRED",
      "This connection was revoked. Create a new authorization explicitly.",
    );
    try {
      ...
      return this.save({
        ...connection,
        state: "unreachable",
        lastError: "The MCP server could not be reached. Check its endpoint and authorization.",
      });
    }
  }
  allowTools(
    ...
    const connection = this.read(principal, id);
    invariant(
      connection.ownerId === principal.id || principal.roles.includes("developer"),
      "ACCESS_DENIED",
      "Only the connection owner or an authorized developer can grant tool access.",
    );
    this.policy.require(principal, connection.spaceId, "write");
    invariant(
      connection.state === "ready" || connection.state === "schema_changed",
      "RECONNECTION_REQUIRED",
      "Reconnect and discover this server before granting tool access.",
    );
    invariant(
      connection.fingerprint === expectedFingerprint,
      "STALE_REVISION",
      "The tool catalog changed. Review the current tools first.",
    );
    invariant(
      names.every((name) => connection.tools.some((tool) => tool.name === name)),
      "INVALID_INPUT",
      "Grant access only to discovered tools.",
    );
    return this.save({
      ...connection,
      allowedTools: [...names],
  ...
  async completeAuthorization(
    ...
    invariant(
      ...
      "ACCESS_DENIED",
      "The connection owner must complete authorization.",
    );
    this.policy.require(principal, connection.spaceId, "write");
    invariant(
      connection.state !== "revoked",
      "RECONNECTION_REQUIRED",
      "Create a new connection after revocation.",
    );
    ...
    this.save({
      ...connection,
      state: "connecting",
      backgroundAccess: credentials.refreshToken ? "supported" : "until_expiry",
    });
  ...
  async call(
    ...
    invariant(
      connection.state === "ready" || connection.state === "refreshing",
      "RECONNECTION_REQUIRED",
      "This connection needs attention before the agent can use it.",
    );
    invariant(
      connection.fingerprint === expectedFingerprint,
      "STALE_REVISION",
      "The MCP tool schema changed. Review the action against the current catalog.",
    );
    invariant(
    ...
    if (
      ...
      if (!credentials.refreshToken || !this.transport.refresh) {
        this.save({
          ...
          backgroundAccess: "until_expiry",
        });
        throw new HarnessFault(
          "RECONNECTION_REQUIRED",
          "The connection expired. Reconnect it to resume this work.",
        );
      }
        ...
        refresh = (async () => {
          try {
            ...
            invariant(
              ...
              "RECONNECTION_REQUIRED",
              "Connection access was revoked during refresh.",
            );
            await this.vault.put(id, {
              ...next,
              ...(next.refreshToken ? {} : { refreshToken: previous.refreshToken! }),
            });
            if (this.store.get<McpConnection>("connections", id)?.state === "revoked") {
              await this.vault.delete(id);
              throw new HarnessFault(
                "RECONNECTION_REQUIRED",
                "Connection access was revoked during credential storage.",
              );
            }
            this.save({
              ...current,
              state: "ready",
              backgroundAccess: next.refreshToken ? "supported" : "until_expiry",
            });
            return next;
          } catch (error) {
            ...
            if (current.state !== "revoked")
              this.save({
                ...current,
                state: "reconnection_required",
                lastError: "Authorization could not be renewed. Reconnect this account.",
              });
            throw new HarnessFault(
              "RECONNECTION_REQUIRED",
              "Authorization could not be renewed. Reconnect this account.",
            );
          } finally {
    ...
    invariant(
      ...
      "RECONNECTION_REQUIRED",
      "The connection is no longer ready.",
    );
    this.policy.require(principal, connection.spaceId, "execute");
    invariant(
      connection.fingerprint === expectedFingerprint && connection.allowedTools.includes(tool),
      "ACCESS_DENIED",
      "Connection grants changed while preparing this operation.",
    );
  ...
  async revoke(principal: Principal, id: string): Promise<void> {
    ...
    invariant(
      connection.ownerId === principal.id || principal.roles.includes("developer"),
      "ACCESS_DENIED",
      "Only the owner or an authorized developer can revoke this connection.",
    );
    this.policy.require(principal, connection.spaceId, "write");
