export type EncodedValue =
  ...
  | number
  | { ref: number }
  | { scalar: "undefined" | "bigint" | "number"; value: string };
...
export interface ValueGraph {
  format: 1;
  roots: Record<string, EncodedValue>;
  nodes: GraphNode[];
}
...
export function encodeGraph(roots: Record<string, unknown>): ValueGraph {
  ...
  const fail = (path: string, what: string): never => {
    throw new Error(
      `[UNSUPPORTED_VALUE] Cannot retain ${path}: ${what}. Store plain data, bytes, or a durable handle instead.`,
    );
  };
  const visit = (value: unknown, path: string): EncodedValue => {
    ...
    if (typeof value === "number")
      return Number.isFinite(value) && !Object.is(value, -0)
        ? value
        : { scalar: "number", value: Object.is(value, -0) ? "-0" : String(value) };
    ...
    if (value instanceof Uint8Array) node = { kind: "bytes", values: Array.from(value) };
      ...
      node = {
        kind: "map",
        entries: Array.from(value, ([k, v], i) => [
          visit(k, `${path}.key[${i}]`),
          visit(v, `${path}.value[${i}]`),
        ]),
      };
    else if (value instanceof Set)
      node = { kind: "set", entries: Array.from(value, (v, i) => visit(v, `${path}[${i}]`)) };
    else {
      const proto = Object.getPrototypeOf(value);
      if (!Array.isArray(value) && proto !== Object.prototype && proto !== null)
        return fail(path, proto?.constructor?.name ?? "class instance");
      ...
      node = Array.isArray(value)
        ...
        : { kind: "object", nullPrototype: proto === null, entries };
    }
    nodes[id] = node;
    return { ref: id };
  };
...
export function decodeGraph(graph: ValueGraph): Record<string, unknown> {
  if (graph.format !== 1 || !Array.isArray(graph.nodes))
    throw new Error("[UNSUPPORTED_VALUE] Unsupported workspace graph version.");
  const values: unknown[] = graph.nodes.map((node) => {
    switch (node.kind) {
      ...
      case "bytes":
        return new Uint8Array(node.values);
    }
  });
  const read = (value: EncodedValue): unknown => {
    ...
    if ("ref" in value) {
      if (!Number.isInteger(value.ref) || value.ref < 0 || value.ref >= values.length)
        throw new Error("[UNSUPPORTED_VALUE] Invalid graph reference.");
  ...
  graph.nodes.forEach((node, i) => {
    if (node.kind === "object" || node.kind === "array") {
      for (const [key, value] of node.entries)
        Object.defineProperty(values[i], key, {
          value: read(value),
          enumerable: true,
          writable: true,
          configurable: true,
      ...
      for (const [key, value] of node.entries)
        (values[i] as Map<unknown, unknown>).set(read(key), read(value));
