...
it("retains binary artifacts and byte ranges without exposing private handles", async () => {
  ...
  expect(
    (await new Artifacts(store, backend).read(buyerA, handle.id, { offset: 1, length: 2 })).bytes,
  ).toEqual(new Uint8Array([255, 42]));
  await expect(artifacts.read(buyerB, handle.id)).rejects.toMatchObject({ code: "NOT_FOUND" });
  await expect(
    artifacts.write(buyerA, { ...input, name: "different" }, [], "operation-1"),
...
it("rechecks revocation after blob upload before publishing its handle", async () => {
  ...
  const artifacts = new Artifacts(store, {
    put: async () => {
      new AccessPolicy(store).setGrants(
        dev,
        "a",
        [{ principalId: dev.id, permissions: ["read", "write", "execute", "publish"] }],
        1,
      );
    },
  ...
  await expect(
    artifacts.write(buyerA, { spaceId: "a", name: "quote.txt", text: "private" }, [], "upload"),
