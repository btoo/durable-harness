...
function setup(kind: LearningTarget["kind"] = "instruction") {
  const {store,close} = database(); cleanup.push(close);
  const target: LearningTarget = {
    name: "quote-normalization", kind,
    validate(value) { invariant(value === "unit" || value === "line", "INVALID_INPUT", "Choose a supported comparison basis."); },
    cases: [
      { id: "case-price", family: "case-pack", split: "validation", input: { price: 120, quantity: 12 }, expected: 10, critical: true },
      { id: "single-unit", family: "unit-pack", split: "validation", input: { price: 19, quantity: 1 }, expected: 19, critical: true },
      { id: "unseen", family: "sealed-pack", split: "heldout", input: { price: 84, quantity: 4 }, expected: 21, critical: true },
    ],
    evaluator: { id: "price-per-unit-v1", async evaluate({configuration,testCase}) {
      ...
      return { caseId: testCase.id, passed, score: Number(passed), explanation: `${actual} per unit; expected ${testCase.expected}` };
    } },
  };
  const learning = new Learning(store, [target]);
  learning.initialize(dev,"a",target.name,"line");
  learning.feedback(buyerA, { id: "correction-1", workspaceId: "a", agent: "quoting", kind: "correction", text: "Compare the price per unit", sourceRunId: "run-1", lineage: [] });
  const proposal = learning.propose(buyerA, { workspaceId: "a", kind, target: target.name, baseRevision: 1, candidate: "unit", rationale: "The original comparison ignored case quantities", evidenceIds: ["correction-1"], lineage: [], origin: "customer_correction" });
