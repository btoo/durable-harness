...
function fixture(transport: McpTransport) {
  ...
  const vault: CredentialVault = new EncryptedCredentialVault(
    new EncryptedSecrets(store, btoa("a".repeat(32))),
  );
  return { store, vault, connections: new Connections(store, vault, transport) };
}
const catalog = [
  { name: "lookup", description: "Look up a synthetic part", inputSchema: { type: "object" } },
];
const input = {
  spaceId: "a",
  name: "Parts",
...
describe("remote connection authorization", () => {
  it("renews once for concurrent unattended calls and restores encrypted credentials", async () => {
    ...
    const transport: McpTransport = {
      ...
      call: async (_connection, _tool, _input, credentials) => {
        used.push(credentials);
        return { part: "ABC" };
      },
    };
    const { store, vault, connections } = fixture(transport);
    const created = await connections.add(buyerA, input, {
    ...
    await Promise.all(
      [1, 2].map(() => connections.call(buyerA, created.id, "lookup", {}, created.fingerprint)),
    );
  ...
  it("requires reconnection when unattended renewal is unavailable", async () => {
    ...
    connections.allowTools(buyerA, created.id, ["lookup"], created.fingerprint);
    await expect(
      connections.call(buyerA, created.id, "lookup", {}, created.fingerprint),
    ).rejects.toMatchObject({ code: "RECONNECTION_REQUIRED" });
  ...
  it("does not resurrect revoked credentials during refresh", async () => {
    ...
    const { connections, vault } = fixture({
      connect: async () => ({ tools: catalog }),
      call: async () => {
  ...
  it("invalidates prepared calls after a remote schema change", async () => {
    ...
    const { connections } = fixture({
      connect: async () => ({
        tools: changed
          ? [{ ...catalog[0]!, inputSchema: { type: "object", required: ["part"] } }]
          : catalog,
      }),
      call: async () => ({}),
    });
