...
describe("code cells in real Dynamic Workers and Durable Object storage", () => {
  ...
  it("stops replay when a pinned tool implementation changed", async () => {
    ...
    const id = crypto.randomUUID();
    const source =
      'const offers = await tools.call("offers", {}); const sent = await tools.call("send", {to:"test@example.test"});';
  ...
  it("uses memory and R2 artifact handles from a restored code cell", async () => {
    ...
    const first = await workspace.run(
      'const saved = await memory.write({title:"Delivery exception",kind:"preference",value:{day:"Tuesday"}}); const file = await artifacts.write({name:"supplier.txt",text:"Requested Tuesday delivery."});',
    );
    expect(first.ok, JSON.stringify(first)).toBe(true);
    await evictDurableObject(workspace);
    const next = await workspace.run(
      "const exception = await memory.read(saved.id); const excerpt = await artifacts.read(file.id, {offset:10,length:7});",
    );
    expect(next.ok, JSON.stringify(next)).toBe(true);
    if (next.ok) {
      expect(next.values.exception).toMatchObject({ value: { day: "Tuesday" } });
      expect(next.values.excerpt).toMatchObject({ text: "Tuesday" });
    }
  });
  it("retains model-shaped data and helpers across eviction", async () => {
    const workspace = host();
    const first = await workspace.run(
      'const offers = await tools.call("offers", {}); const evidence = { offers }; const alias = evidence; function cheapest(items: {price: number}[]) { return items.reduce((a, b) => a.price < b.price ? a : b); }',
    );
    expect(first.ok, JSON.stringify(first)).toBe(true);
    await evictDurableObject(workspace);
    const next = await workspace.run(
      "const best = cheapest(offers); const same = alias === evidence;",
    );
    expect(next.ok, JSON.stringify(next)).toBe(true);
    if (next.ok) {
      expect(next.values.best).toEqual({ supplier: "B", price: 95 });
      expect(next.values.same).toBe(true);
      expect(next.workspace.revision).toBe(2);
    }
    expect(await workspace.counts()).toEqual({ offers: 1, sends: 0 });
  });
  it("resumes an approval after eviction without repeating completed reads", async () => {
    ...
    const id = crypto.randomUUID();
    const source =
      'const offers = await tools.call("offers", {}); const delivery = await tools.call("send", {to: "buyer@example.test"});';
  ...
  it("rejects captured mutable state and leaves the workspace unmodified", async () => {
    ...
    const result = await workspace.run(
      "const rate = 2; function normalize(price: number) { return price * rate; }",
    );
    expect(result.ok).toBe(false);
  ...
  it("pins retained helper dependencies across later redefinitions", async () => {
    ...
    expect(
      (
        await workspace.run(
          "function rate(n: number) { return n * 2; } function quote(n: number) { return rate(n); }",
        )
      ).ok,
    ).toBe(true);
    ...
    if (result.ok) {
      expect(result.values.existing).toBe(20);
      expect(result.values.updated).toBe(30);
    }
  });
  ...
  it("reconstructs audience-filtered events with stable cursors", async () => {
    const workspace = host();
    await workspace.run(
      'const offers = await tools.call("offers", {}); await runtime.progress("Two supplier responses are available.");',
    );
    const visible = await workspace.events(customer);
    expect(visible.some((event) => event.kind === "cell.started")).toBe(false);
    expect(visible.some((event) => event.text === "Two supplier responses are available.")).toBe(
      true,
    );
