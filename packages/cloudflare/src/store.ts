...
export class SqlRecordStore implements RecordStore {
  constructor(private readonly db: SqlDatabase) {
    ...
    db.exec(
      "CREATE VIRTUAL TABLE IF NOT EXISTS dh_search USING fts5(collection UNINDEXED, id UNINDEXED, text)",
    );
  }
  get<T>(collection: string, id: string): T | undefined {
    const row = [
      ...this.db.exec(
        "SELECT value FROM dh_records WHERE collection = ? AND id = ?",
        collection,
        id,
      ),
    ][0];
  ...
  put<T>(collection: string, id: string, value: T): void {
    ...
    this.db.exec(
      "INSERT INTO dh_records (collection,id,value) VALUES (?,?,?) ON CONFLICT(collection,id) DO UPDATE SET value = excluded.value",
      collection,
      id,
      encoded,
    );
    if (collection === "history") {
      ...
      this.db.exec(
        "INSERT INTO dh_search (collection,id,text) VALUES (?,?,?)",
        collection,
        id,
  ...
  list<T>(collection: string): T[] {
    return [
      ...this.db.exec(
        "SELECT value FROM dh_records WHERE collection = ? ORDER BY rowid",
        collection,
      ),
  ...
  search<T>(collection: string, query: string, allowedIds: string[], limit: number): T[] {
    ...
    return [
      ...this.db.exec(
        "SELECT r.value FROM dh_search f JOIN dh_records r ON r.collection = f.collection AND r.id = f.id WHERE dh_search MATCH ? AND f.collection = ? AND f.id IN (SELECT value FROM json_each(?)) ORDER BY bm25(dh_search) LIMIT ?",
        expression,
        collection,
