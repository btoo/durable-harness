...
export class History {
        ...
        if (existing) {
          invariant(
            existing.workspaceId === item.workspaceId &&
              ...
              JSON.stringify(existing.lineage) === JSON.stringify(item.lineage) &&
              JSON.stringify(existing.metadata) === JSON.stringify(item.metadata),
            "REPLAY_DIVERGENCE",
            "This history ID already identifies a different event.",
          );
          return existing;
        }
      }
  ...
  list(principal: Principal, workspaceId: string): HistoryItem[] {
    ...
    return this.store
      ...
      .filter((item) => item.workspaceId === workspaceId && this.canRead(principal, item))
  ...
  read(principal: Principal, id: string): HistoryItem {
    ...
    invariant(
      item && this.canRead(principal, item),
      "NOT_FOUND",
      "No accessible history item exists with that reference.",
    );
    return item;
  }
  ...
  search(principal: Principal, query: string, workspaceIds: string[], limit = 10): HistoryItem[] {
    ...
    return visible
      .map((item) => ({
        item,
        score: terms.reduce(
          (score, term) => score + Number(item.text.toLocaleLowerCase().includes(term)),
          0,
        ),
      }))
  ...
  private canRead(principal: Principal, item: HistoryItem): boolean {
    return (
      (item.audience === "customer" || principal.roles.includes("developer")) &&
      this.access.visible(principal, item.workspaceId, item.lineage)
    );
  }
}
    ...
    return this.store
      ...
      .filter(
        (event) =>
          ...
          (event.audience === "customer" || principal.roles.includes("developer")) &&
          access.visible(principal, workspaceId, event.lineage),
      )
      .sort((a, b) => a.sequence - b.sequence);
  }
}
